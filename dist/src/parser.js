    ...
    switch (typeOfSchema_1.typeOfSchema(schema)) {
        case 'ALL_OF':
            return set({
                ...
                keyName: keyName,
                params: schema.allOf.map(function (_) {
                    return parse(_, options, rootSchema, undefined, true, processed, usedNames);
                }),
        ...
        case 'ANY_OF':
            return set({
                comment: schema.description,
                keyName: keyName,
                params: schema.anyOf.map(function (_) {
                    return parse(_, options, rootSchema, undefined, true, processed, usedNames);
                }),
                standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
                type: 'UNION'
        ...
        case 'TYPED_ARRAY':
            if (Array.isArray(schema.items)) {
                return set({
                    comment: schema.description,
                    keyName: keyName,
                    params: schema.items.map(function (_) {
                        return parse(_, options, rootSchema, undefined, true, processed, usedNames);
                    }),
                    standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
                    type: 'TUPLE'
        ...
        case 'UNION':
            return set({
                ...
                params: schema.type.map(function (_) {
                    return parse({ type: _ }, options, rootSchema, undefined, true, processed, usedNames);
                }),
                standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
                type: 'UNION'
            });
        case 'UNNAMED_ENUM':
            return set({
                comment: schema.description,
                keyName: keyName,
                params: schema.enum.map(function (_) {
                    return parse(_, options, rootSchema, undefined, false, processed, usedNames);
                }),
                standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
                type: 'UNION'
            });
...
function parseSuperTypes(schema, options, processed, usedNames) {
    ...
    if (Array.isArray(superTypes)) {
        return superTypes.map(function (_) {
            return newNamedInterface(_, options, _, processed, usedNames);
        });
    }
    return [
        newNamedInterface(superTypes, options, superTypes, processed, usedNames)
    ];
}
